...
CKEDITOR.dialog.add( 'pagelink', function( editor ) {
	...
	var targetChanged = function() {
			...
			switch ( value ) {
				case 'frame':
					targetName.setLabel( editor.lang.pagelink.targetFrameName );
					targetName.getElement().show();
					break;
				case 'popup':
					popupFeatures.show();
					targetName.setLabel( editor.lang.pagelink.targetPopupName );
					targetName.getElement().show();
					break;
				default:
	...
	var linkTypeChanged = function() {
			var dialog = this.getDialog(),
				partIds = [ 'urlOptions',  'pagelinkOptions', 'anchorOptions', 'emailOptions' ],
	...
	var parseLink = function( editor, element ) {
			...
			if ( !retval.type ) {
				...
				// urlRegex matches empty strings, so need to check for href as well.
					...
					retval.type = 'url';
					if (urlMatch[ 2 ].search(/\[pagelink/) === 0) retval.type = 'pagelink';
					retval.url = {};
			...
			if ( element ) {
				...
				CKEDITOR.plugins.pagelink.synAnchorSelector ? retval.adv.advCSSClasses = getLinkClass( element ) : advAttr( 'advCSSClasses', 'class' );
			...
			if ( CKEDITOR.plugins.pagelink.emptyAnchorFix ) {
			...
			if ( CKEDITOR.plugins.pagelink.fakeAnchor ) {
				var imgs = editor.document.getElementsByTag( 'img' );
				for ( i = 0, count = imgs.count(); i < count; i++ ) {
					if ( ( item = CKEDITOR.plugins.pagelink.tryRestoreFakeAnchor( editor, imgs.getItem( i ) ) ) )
	...
	return {
				...
				children: [
					...
					children: [
						...
						commit: function( data ) {
							if ( !data.pagelink )
								data.pagelink = {};
							data.pagelink.url = this.getValue();
						}
					}
					],
					setup: function( data ) {
		...
		onOk: function() {
				...
				case 'pagelink':
				  if (data.newslink && data.newslink.url !== "") {
				    var url = ( CKEDITOR.tools.trim( data.newslink.url ) );
					  ...
					  var url = ( CKEDITOR.tools.trim( data.pagelink.url ) ) || '';
					}
			...
			if ( !this._.selectedElement ) {
				...
				if ( data.adv && data.adv.advName && CKEDITOR.plugins.pagelink.synAnchorSelector )

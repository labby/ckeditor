...
CKEDITOR.dialog.add( 'anchor', function( editor ) {
	...
	return {
		title: editor.lang.pagelink.anchor.title,
		minWidth: 300,
		minHeight: 60,
		onOk: function() {
		...
		onShow: function() {
			...
			if ( fullySelected ) {
				if ( CKEDITOR.plugins.pagelink.fakeAnchor ) {
					var realElement = CKEDITOR.plugins.pagelink.tryRestoreFakeAnchor( editor, fullySelected );
				...
				partialSelected = CKEDITOR.plugins.pagelink.getSelectedLink( editor );
				if ( partialSelected ) {
		...
		contents: [
			{
			id: 'info',
			label: editor.lang.pagelink.anchor.title,
			accessKey: 'I',
			elements: [
				{
				type: 'text',
				id: 'txtName',
				label: editor.lang.pagelink.anchor.name,
				required: true,
				validate: function() {
					if ( !this.getValue() ) {
						alert( editor.lang.pagelink.anchor.errorName );
						return false;
					}
					return true;
				}
